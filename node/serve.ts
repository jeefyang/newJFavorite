    ...
    init() {
        ...
        this.server = http.createServer((req, res) => {
            ...
            if (urlParase.pathname == "/") {
                ...
                const folder = a.searchParams.get("folder") || "";
                const folderData = this.getHistoryList(folder);
                console.log(folderData);
                let favoriteData = "";
                let curFavriteFilePath = folderData.favoriteFilePath;
                if (curFavriteFilePath) {
                    curFavriteFilePath = path.join(this.config.favoriteDir, folder, a.searchParams.get("file") || "");
                ...
                indexHtml = indexHtml.replace("$getfavoritesUrl_serve$.data", `\`${favoriteData ? encodeURIComponent(favoriteData) : ""}\``);
                indexHtml = indexHtml.replace("$getfavoritesUrl_serve$.type", `\`${curFavriteFilePath ? path.extname(curFavriteFilePath).replace(".", "").toLocaleLowerCase() : ""}\``);
            ...
            // 历史列表
                ...
                const folder = a.searchParams.get("folder") || "";
                const folderData = this.getHistoryList(folder);
                res.write(JSON.stringify(folderData.list));
                res.end();
            }
            // 上传
            else if (urlParase.pathname == "/upload") {
                console.log(urlParase.pathname);
                //   res.writeHead(200,{})
                //新建一个空数组接受流的信息
                let chunks: any[] = [];
                //获取的长度
                ...
                req.on("end", () => {
                    ...
                    const folder = a.searchParams.get("folder") || "";
                    const folderUrl = path.join(this.config.favoriteDir, folder);
                    if (!fs.existsSync(folderUrl)) {
                        fs.mkdirSync(folderUrl, { recursive: true });
                    }
                    const fileUrl = path.join(folderUrl, newFileName);
                    console.log(`上传文件全路径:${fileUrl}`);
